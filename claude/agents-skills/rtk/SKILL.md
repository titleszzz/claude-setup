...
3. If the command fails with "command not found", tell the user RTK is not on PATH here (see `~/.Codex/RTK.md` — a name collision with a different `rtk` is the usual cause) rather than guessing at numbers.
