...
```bash
bash ~/.Codex/skills/handoff/scripts/handoff-path.sh
```

It prints `~/.Codex/projects/<encoded-cwd>/memory/handoff-state.md`.
...
Run `node ~/.Codex/skills/handoff/scripts/check-handoff.mjs --verbose` from the
...
- This is a manual command. Codex cannot see its own context percentage and cannot fire this at a threshold. To run it automatically before auto-compaction, register a `PreCompact` hook in `~/.Codex/settings.json`.
